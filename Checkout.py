...
def update_attendance(mssv, time_check_out, trang_thai):
    """Cập nhật thông tin điểm danh vào cơ sở dữ liệu."""
    result = db.sukien.update_one(
        {'mask': '12DHTH13'},
        {
            '$set': {
                f'dssinhvien_thamgia.$[elem].tgiancheck_out': time_check_out,
...
def load_data_by_mssv(mssv):
    ...
    if student:
        ...
        update_attendance(mssv, check_out_time, 'Đã điểm danh ra')

        # Thông tin đầy đủ được lưu trữ và cập nhật vào TreeView
        date, time = check_out_time.split()
        save_attendance(student['hoten'], student['lop'], student['mssv'], date, time, "Đã điểm danh ra")
...
def open_attendance_window():
    ...
    global treeview
    treeview = ttk.Treeview(attendance_window, columns=("Họ tên", "Lớp", "MSSV", "Ngày ra", "Giờ ra", "Trạng thái"),
                            show="headings")
    treeview.heading("Họ tên", text="Họ tên")
    treeview.heading("Lớp", text="Lớp")
    treeview.heading("MSSV", text="MSSV")
    treeview.heading("Ngày ra", text="Ngày ra")
    ...
    checked_out_students = len(recognized_students)
    label_attendance_info = ttk.Label(attendance_window,
                                      text=f"Số sinh viên đã điểm danh: {checked_out_students}/{total_students}")
    ...
    for event in events.find():
        if event['mask'] == '12DHTH13':
            for student in event['dssinhvien_thamgia']:
                if student['tgiancheck_out']:
                    date, time = student['tgiancheck_out'].split()
...
def export_attendance_to_csv():
    ...
    try:
        with open("diemdanh_data.csv", "w", newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(["Họ tên", "Lớp", "MSSV", "Ngày ra", "Giờ ra", "Trạng thái"])
